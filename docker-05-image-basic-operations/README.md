...
@app.route("/")
def hello():
    return "<h1>Welcome to Ali Fuat</h1>"
...
docker build -t "springbluebay/123" .
...
docker run -d --name welcome3 -p 84:80 springbluebay/123
...
```bash
docker push callahanclarus/flask-app:1.0
```
